...
temp1 = df['salary_low'].str.extract('(\d+)k').applymap(lambda x: int(x))
df['year_salary_low'] = temp1[0] * df['salaryMonth']
temp2 = df['salary_high'].str.extract('(\d+)k').applymap(lambda x: int(x))
df['year_salary_high'] = temp2[0] * df['salaryMonth']
# #处理格式
df['salary_mean'] = df['salary_mean'].apply(lambda x: str(x) + 'k')
df['year_salary_mean'] = df['year_salary_mean'].apply(lambda x: str(x / 10) + 'w')
df['year_salary_low'] = df['year_salary_low'].apply(lambda x: str(x / 10) + 'w')
df['year_salary_high'] = df['year_salary_high'].apply(lambda x: str(x / 10) + 'w')
df.to_csv('/Users/zhouya/Documents/code/analyze-lagou/data/tableauData.csv', sep='\t', index=False)
